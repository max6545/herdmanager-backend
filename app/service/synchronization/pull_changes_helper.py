...
def get_pull_changes(watermelon_class: WatermelonModel, changelog_class: ChangeLog,
                     timestamp_as_datetime: datetime, user_id: int, migration_number: int = 11):
    return {
        'created': get_created_objects(watermelon_class, timestamp_as_datetime, user_id, migration_number),
        'updated': get_updated_objects(watermelon_class, timestamp_as_datetime, user_id, migration_number),
        'deleted': get_deleted_object_ids(changelog_class, timestamp_as_datetime, user_id)
    }


def get_created_objects(class_name: WatermelonModel, timestamp_as_datetime: datetime, user_id: int,
                        migration_number: int = 11):
...
def get_updated_objects(class_name: WatermelonModel, timestamp_as_datetime: datetime, user_id: int,
                        migration_number: int = 11):
    farm_id = User.query.filter_by(id=user_id).first().farm_id
...
def get_deleted_object_ids(class_name: ChangeLog, timestamp_as_datetime: datetime, user_id: int):
    farm_id = User.query.filter_by(id=user_id).first().farm_id
